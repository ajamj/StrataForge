...
```rust
use sf_ml::data::SyntheticGenerator;
...
```rust
use sf_ml::{AutoTracker, TrackerConfig};
...
```rust
use sf_ml::search::{SearchStrategy, AdaptiveSearch};
...
```rust
use sf_ml::parallel::ParallelTracker;
...
```rust
use sf_ml::data::{TrainingDataset, DataAugmentation};
...
```rust
use sf_ml::training::{ModelTrainer, TrainingConfig, Optimizer};
...
```rust
use sf_ml::training::TrainingCallback;
...
```rust
use sf_ml::tuning::HyperparameterSearch;
...
```python
from stratforge.ml import AutoTracker, HorizonCNN, TrackerConfig

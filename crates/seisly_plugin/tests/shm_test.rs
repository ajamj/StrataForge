...
fn test_shm_basic_transfer() {
    ...
    let actual_sum: f64 = match &sum_result {
        ...
        _ => panic!("Expected number or string result containing sum, got: {:?}", sum_result),
    };
...
fn test_shm_cross_process() {
    ...
    assert!(result.is_ok(), "Large SHM transfer failed: {:?}", result.err());
...
fn test_shm_performance_vs_json() {
    ...
    assert!(shm_elapsed.as_secs() < 1, "SHM transfer too slow: {:?}", shm_elapsed);
}

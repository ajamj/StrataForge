...
"#;
    ...
    let result = manager.execute("TestPythonPlugin", "run", serde_json::Value::Null).unwrap();
    assert_eq!(result, serde_json::Value::String("Python execution successful".to_string()));
}

...
impl LasWriter {
    ...
    pub fn write(well: &Well, path: &Path) -> Result<(), LasWriteError> {
        ...
        writeln!(writer, " VERS.                          2.0:   CWLS LOG ASCII STANDARD -VERSION 2.0")?;
        writeln!(writer, " WRAP.                          NO:   ONE LINE PER DEPTH STEP")?;
        writeln!(writer, " DLM .                          SPACE:   DELIMITER")?;
        writeln!(writer)?;

        // Write Well Section
        writeln!(writer, "~WELL INFORMATION")?;
        writeln!(writer, " STRT.{}{:12.4}:                :START DEPTH", depth_unit, min_depth)?;
        writeln!(writer, " STOP.{}{:12.4}:                :STOP DEPTH", depth_unit, max_depth)?;
        writeln!(writer, " STEP.{}{:12.4}:                :STEP", depth_unit, 0.5)?;
        writeln!(writer, " NULL.                     -999.25:                :NULL VALUE")?;
        writeln!(writer, " WELL.                 {:20}:                :WELL NAME", well.name)?;
        writeln!(writer, " API .                              :                :API NUMBER")?;
        writeln!(writer)?;
        ...
        // Depth curve
        writeln!(writer, " DEPT.{}                      : 1  :DEPTH", depth_unit)?;
        
        // Log curves
        for (i, log) in well.logs.iter().enumerate() {
            let desc = if log.description.is_empty() { &log.mnemonic } else { &log.description };
            writeln!(
                writer,
                " {:6}.{:8}                  : {:2}  :{}",
                log.mnemonic,
        ...
        writeln!(writer, " MUD .                              :                :MUD TYPE")?;
        writeln!(writer, " BHT .                              :                :BOTTOM HOLE TEMP")?;
        writeln!(writer)?;

        // Write Data Section
        writeln!(writer, "~A  DEPTH     {}", 
            well.logs.iter().map(|l| &l.mnemonic[..6.min(l.mnemonic.len())]).collect::<Vec<_>>().join("        ")
        )?;
...
mod tests {
    ...
    fn create_test_well() -> Well {
        let mut well = Well::new("Test Well".to_string(), "TW-1".to_string(), 500000.0, 1000000.0, 50.0);
